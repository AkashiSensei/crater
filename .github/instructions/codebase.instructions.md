...
- **作业模板兼容性**: 修改作业创建配置字段、模板持久化结构、模板序列化或克隆作业相关 payload 时，必须判断是否提升对应前端 `MetadataForm*` version；旧配置仍需可用时必须补“上一版本 -> 新版本”的迁移函数并接入共享模板导入 / 迁移逻辑，后续旧版本通过迁移链升级，不支持的更早版本要清晰阻断。
