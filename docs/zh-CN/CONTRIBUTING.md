...
Commit subject 格式为 `type: subject` 或 `type(scope): subject`，其中 `scope` 可选，可用于说明改动范围，例如 `docs(cli): add command examples`。该格式只约束 commit message 的第一行；完整 commit message 可以包含正文和 `Signed-off-by` 等 trailer。
...
若有多个 commit，则每个 commit subject 分别按上表编写 `type(scope): subject` 格式。

新 commit 的 subject 必须使用以下形式之一：
...
允许的 type 为 `feat`、`fix`、`docs`、`style`、`refactor`、`test`、`chore`。scope 可选，但建议用于说明改动范围，例如 `docs(cli): add command examples`。

subject 格式只约束第一行。如果 commit 需要更多上下文，可在 subject 后空一行再写正文。DCO sign-off 属于 commit trailer，应放在正文之后，例如：
...
向 Crater 提交贡献，即表示你同意该贡献按 Apache License, Version 2.0 授权给本项目，除非你明确另行声明。同时，你确认自己有权提交该贡献。

DCO sign-off 推荐所有贡献者使用；非 The Crater Project Team 成员的外部贡献者必须使用。添加 sign-off 表示你确认自己有权提交该贡献，并同意该贡献按 Apache License, Version 2.0 授权给本项目。

提交时可使用 `git commit -s` 添加 sign-off：

```bash
git commit -s -m "feat(portal): add job submission form"
```

如果最新 commit 忘记添加 sign-off，可用以下命令补充：

```bash
git commit --amend -s
```

Crater 当前不会通过自动 DCO workflow 拒绝 Pull Request。维护者仍可要求外部贡献者在合并前补充或确认 sign-off。

维护者 squash merge PR 时，必须在最终 squash commit message 中保留已有的 `Signed-off-by` 行，且不要重复添加。维护者不得替贡献者新增 `Signed-off-by`，除非相同 sign-off 已经出现在贡献者提交中，或贡献者在 PR 中明确确认。若维护者在合并时做了实质改动，应添加维护者自己的 sign-off。

自动化或 Agent 工作流代开发者创建 commit 时，默认应使用 `git commit -s`，但不得静默添加 sign-off。创建 commit 前，Agent 必须向开发者解释 `Signed-off-by` 是 DCO 声明，表示开发者确认自己有权按项目许可证提交该贡献；随后必须展示完整 commit message，包括所有 `Signed-off-by` 行，供开发者检查确认。拟使用的 subject 也必须符合上文 commit subject 格式。
...
- 后端常量位于 `backend/internal/version/api.go`，CLI 常量位于 `cli/internal/version/version.go`。
- `APIVersion` 是双方共享、只增不减的 API 契约计数器。CLI 所用的 API 契约发生变化后，后端与 CLI 必须携带相同的当前值。
- `1` 是首个正式契约版本；`0` 不得作为受支持的正式契约版本发布。握手接口明确报告 `0` 时，CLI 将其视为早于正式契约的旧版本，以便说明哪个最低版本条件不满足；字段缺失或负数仍表示未知或非法。
- 双方分别维护自己所需的最低对方版本：后端维护 `MinSupportedCLIAPIVersion`，CLI 维护 `MinSupportedBackendAPIVersion`。两者不要求相等，也不能仅因 `APIVersion` 增长就自动提升。
- 只有一方在没有回退路径时确实无法与旧版对方完成受影响操作，才提升该方维护的最低版本；其值应为首次提供所需契约的对方 API 版本。
- 每个修改 CLI 所用 API 的 PR，都必须在描述或验证记录中写明版本决策；不调整时也要写明“不变”及原因。

| 变更 | `APIVersion` | 最低对方版本 |
|------|--------------|--------------|
| 仅后端实现、仅前端使用的 API 或仅 CLI 本地行为 | 不变 | 不变 |
| 新增 CLI 会消费的接口、字段或可选语义 | 后端与 CLI 同时加一 | 通常不变 |
| 修改或删除 CLI 所用的路径、方法、必填字段、字段类型、含义、鉴权要求或响应结构 | 后端与 CLI 同时加一 | 仅无法再支持旧版对方的一侧提升 |
| CLI 开始强依赖后端 API 契约中已经存在的能力 | 不变 | 没有回退路径时，将 CLI 的最低后端版本提升至首次提供该能力的 API 版本 |
| CLI 增加回退路径或保持已有能力可选 | 后端 API 契约本身未变化时不变 | 不提升最低版本 |

CLI 请求携带 `User-Agent: crater-cli/<product-version>` 与 `X-Crater-API-Version: <APIVersion>`。这些 Header 是不可信的诊断信息；后端不得据此鉴权、判断兼容性、切换响应结构或强制拒绝请求。只有显式执行 `crater compatibility` 诊断命令时才调用公开的 `GET /api/cli/compatibility`；普通业务命令不执行握手，也不得受兼容性结果门禁。
