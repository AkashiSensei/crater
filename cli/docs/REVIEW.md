...
- CLI 所用 API 发生变化时，确认 PR 按根 `CONTRIBUTING.md` 明确记录 `APIVersion` 与 `MinSupportedBackendAPIVersion` 的调整决定；当前版本由后端与 CLI 同步维护，最低版本只在无回退路径的强依赖出现时提升，二者不能机械联动。
- 确认 `User-Agent` 与 `X-Crater-API-Version` 只作诊断，后端没有据此拦截或切换响应；普通业务命令不自动握手，只有 `crater compatibility` 显式诊断，并在版本范围不兼容或握手接口 404 时稳定返回 `ERR_API_VERSION_MISMATCH`。接口明确报告的 `0` 应作为前契约旧版本参与比较并输出完整诊断，不能与字段缺失混为泛化的非法响应；404 的 `0` 占位仍显示未知。后端产品版本、短提交 SHA、构建类型和构建时间必须在普通与 JSON 输出中稳定展示，但只作诊断，缺失时显示未知且不得改变兼容性状态。人类可读提示须分段列出具体失败条件、双方当前和最低支持版本，以及可继续使用的风险说明；不得把业务错误优先归因于版本不兼容，应先提示排查该操作的报错、输入、配置、认证、网络、平台状态和 CLI 缺陷，仅在排除这些原因后才按方向把升级或降级 CLI 作为最后手段。CLI 低于后端要求则升级，后端低于 CLI 要求或接口 404 则降级，两项限制同时不满足则说明方向冲突并要求向管理员确认匹配版本。404 的特殊映射不得影响其他命令，且 context 必须保留原始 `http_status` 与 `msg`。
